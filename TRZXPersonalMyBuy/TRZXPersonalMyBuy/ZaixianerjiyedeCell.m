...
-(void)setCourse:(ZXShouMode *)course{

    self.titleLabel.text = course.name;
    self.kanguoLab.text = [NSString stringWithFormat:@"%ld人看过",(long)course.clickRate];
    [self.iconImageView sd_setImageWithURL:[NSURL URLWithString:course.userPic] placeholderImage:[UIImage imageNamed:@"展位图"]];
    NSString *company = [NSString stringWithFormat:@"%@,",course.company];
    NSString * companyPosition = [NSString stringWithFormat:@"%@%@",company,course.title];
    self.nameLabel.text = course.user;
    self.positionLabel.text = companyPosition;
}
